...
class SCS
{
    public: 
        ...
        void SCS_WMM_Beam_Search_Algorithm()
        {
            printAlphabetAndSequences();
            fResults << "------------------" << std::endl;
            fResults << "SCS_WMM_Beam_Search_Algorithm" << std::endl;
            time_t start, end;
            time(&start);
            ...
            fResults << "-WMM Beam Search solution-" << std::endl;
            fResults << "SCS : " + shortestCommonSupersequence << std::endl;
            fResults << "SCS length : " << shortestCommonSupersequenceLen << std::endl;
    ...
    private: 
        ...
        */
        ...
        std::vector<std::pair<char, std::size_t>> WMM(std::pair<std::string, std::pair<size_t, std::vector<std::size_t>>> &s)
        {
            std::vector<std::pair<char, std::size_t>> letterRanks;
            for(auto &c : alphabet)
            {
            ...
            {
                std::vector<std::size_t> tmpPositions = s.second.second;
                std::size_t lastIndex = setOfStringsVec[i].size() - 1;
                if(tmpPositions[i] <= lastIndex)
                {
                    for(auto &lr : letterRanks)
                    {
                        if(lr.first == setOfStringsVec[i][tmpPositions[i]])
                        {
                            lr.second += (lastIndex - tmpPositions[i]);
                        }
                    }
                }
            }            
