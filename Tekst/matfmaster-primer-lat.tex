...
\begin{algorithm}
  ...
  \State $b1 \gets \{ \}$
  \State $b2 \gets \{ \}$
  \State $\textrm{pronađenaNadniska} \gets \textrm{Netačno}$
  \State
  \State $\textrm{težine} \gets \textbf{H}\textrm{([0,...,0])}$
  \State
  \For{$\alpha\in\Sigma$}
    \State $\textrm{težinaSlova} \gets 0$
    \For{$t\in\textrm{težine}$}
      \If{$\alpha == \textrm{t.slovo}$}
        \State $\textrm{težinaSlova} \gets \textrm{t.vrednost}$
      \EndIf
    \EndFor
    \State $\textrm{element} \gets \{\alpha\textrm{, težinaSlova, [0,...,0]}\}$
    \State $b1\textrm{.dodaj(element)}$
  \EndFor
  \State
  \State $b1 \gets \textbf{RedukujBim}(b1\textrm{, pronađenaNadniska})$
  \State
  \While{$\neg\textrm{pronađenaNadniska}$} 
    \For{$s\in b1$}
      \State $\textrm{težine} \gets \textrm{H(s.pozicije)}$
      \State
      \If{$\textrm{težine[0].vrednost == 0}$}
        \State $\textrm{SkiniPreostalaSlova()}$
        \State $\textrm{pronađenaNadniska} \gets \textrm{Tačno}$
        \State \textbf{break}
      \EndIf
      \State
      \For{$\alpha\in\Sigma$}
        ...
        \For{$t\in\textrm{težine}$}
          ...
          \EndIf
        \EndFor
      \State $\textrm{element} \gets \{\textrm{(s.reč)}\alpha\textrm{, s.hvrednost + težinaSlova, s.pozicije}\}$
      \State $b2\textrm{.dodaj(element)}$
      \EndFor
    \EndFor
    \State
    \State $b1\textrm{.obriši()}$
    \State $b1 \gets \textbf{RedukujBim}(b2\textrm{, pronađenaNadniska})$
    \State $b2\textrm{.obriši()}$
  \EndWhile
  \end{algorithmic}
  \end{algorithm}
\\
...
\\
\begin{algorithm}
  \caption{RedukujBim($\mathcal{B}$, pronađenaNadniska)}
  \label{alg:redukujbim}
  \begin{algorithmic}[1]
    \If{$\textrm{s.veličina()} \leqslant \beta$}
      \For{$e \in \mathcal{B}$}
        \State $\textrm{AžurirajPozicije}(e)$
          \If{$\textrm{ZajedničkaNadniska(e.reč)}$}
            \State $nn \gets \textrm{s.reč}$ \Comment{nn - najbolja nadniska}
            \State $nd \gets |nn|$ \Comment{nd - najbolja dužina}
            \State $\textrm{pronađenaNadniska} \gets \textrm{Tačno}$
          \EndIf
      \EndFor
    ...
    \State $\mathcal{B}p \gets \{ \}$
    \State
    \For{$i=0 \to \beta$}
      \State $\textrm{AžurirajPozicije}(\mathcal{B}[i])$
      \If{$\textrm{ZajedničkaNadniska}(\mathcal{B}[i].\textrm{reč})}$
            \State $nn \gets \mathcal{B}[i].\textrm{reč}$
            \State $nd \gets |nn|$ 
            \State $\textrm{pronađenaNadniska} \gets \textrm{Tačno}$
      \EndIf
      \State $\mathcal{B}p\textrm{.dodaj}(\mathcal{B}[i])$
    \EndFor
    \State
    \State \Return $\mathcal{B}p$
  \end{algorithmic}
  \end{algorithm}
\\
% Algoritam prima tri ulazna parametra: azbuku $\Sigma$, ulazni skup reči $\mathcal{L}$ i širinu bima $\beta$.
