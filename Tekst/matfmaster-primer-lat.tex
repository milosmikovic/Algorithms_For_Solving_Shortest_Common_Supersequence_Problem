...
\begin{figure}[!ht]
  ...
  \caption{Stavi sliku beam searcha} 
...
\begin{algorithm}
  ...
  \While{$True$} \Comment{Može postojati uslov izlaska iz petlje pre pronađenog rešenja}
...
slova iz azbuke. Funkcija OceniBim predstavlja heurističku funkciju koja dodljuje određene vrednosti svakom elementu bima,
...
selekcije (\textit{eng.} roulette selection). Algoritam se može zaustaviti kada se pronađe prvo rešenje problema, ali
mogu se postaviti i drugi uslovi zaustavljanja poput broja iteracija petlje (predstavlja dubinu unutar stabla pretrage)
ili vremenska ograničenja.
...
\begin{algorithm}
  ...
  % \Ensure $y = x^n$
  \State $d \gets |\omega|$
  \If{$(d \geqslant (maxD + 1)) \lor (nd \leq d)$}
      \State \Return
  \EndIf

  \If{$(d \geqslant minD ) \land  daLiJeNadniska(\omega)$}
      \State $nn = \omega$ \Comment{nn - najbolja nadniska}
      \State $nd = |\omega|$ \Comment{nn - najbolja nadniska}
  \EndIf
  \While{$\sum_{\omega_{i}\in\mathcal{L}}|\omega_{i}| \neq 0$} 
  \For{$\alpha \in \Sigma$}
        \State $v(\alpha|\mathcal{S}) \gets \sum_{\omega_{i}\in\mathcal{L},\omega_{i}=\alpha\omega^{'}_{i}}1$
