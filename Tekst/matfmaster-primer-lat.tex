...
Na primer ako najduža reč u skupu $\mathcal{L}$ ima petnaest slova i pritom je
data proizvoljna azbuka kardinalnosti $|\Sigma|=4$, u najgorem slučaju
algoritam grananja sa odsecanjem treba da obradi $(4^{60}-1)/3$ čvorova.
...
Sada će biti definisan skup instanci problema $IP$ koji je korišćen za testiranje u sekciji ref.
...
Na kraju verovatnoća uklanjanja slova uzeta je iz opsega $\gamma \in \{0.1,0.2\}$.
Instance problema podeljene su u dva skupa $IP_{\gamma=0.1}$ i $IP_{\gamma=0.2}$
na osnovu verovatnoće uklanjanja slova. Oba skupa sadrže po 48 instanci problema
koje su dobijene kao dekartov proizvod vrednosti prethodno opisanih parametara.
Rezultati izvršavanja algoritama na uvedenim instancama PNZN 
prikazani su u tabelama ref ref.
...
Prevođenje i pokretanje odgovarajućih programa vrši se na sledeći način:
\begin{itemize}
  ...
  \item ./testInstanceGenerator - pokreće generator test instanci koji nakon poziva 
  ...
  izlaz je numerisan tekstualni fajl koji predstavlja jednu instancu PNZN  
  ...
  koji sadrži rezultat izvršavanja
\end{itemize}
...
Vrednosti širine bima uzete su iz opsega $\beta \in \{50,100,200\}$.
